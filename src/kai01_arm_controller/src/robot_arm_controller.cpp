...
namespace robot_arm_controller
{
  RobotArmController::RobotArmController()
      : controller_interface::ControllerInterface(),
        rt_group_command_ptr_(nullptr),
        group_command_subscriber_(nullptr)
  {
  }
  ...
  {
    param_listener_ =
        std::make_shared<robot_arm_controller::ParamListener>(get_node());
  }
      ...
      const rclcpp_lifecycle::State & /*previous_state*/)
  {
    ...
    RCLCPP_INFO(get_node()->get_logger(),
                "RobotArmController: services ~/set_joint_angles and ~/get_joint_angles created");
      ...
      const rclcpp_lifecycle::State & /*previous_state*/)
  {
    ...
    // Write initial pose to hardware
    if (!params_.init_arm_pose.empty())
    {
      write_pose_to_hardware(params_.init_arm_pose, "init_arm_pose");
    }
  ...
  controller_interface::CallbackReturn RobotArmController::on_deactivate(
      const rclcpp_lifecycle::State & /*previous_state*/)
  {
    // Write deactivate pose to hardware before releasing interfaces
    if (!params_.deactivate_arm_pose.empty())
    {
      write_pose_to_hardware(params_.deactivate_arm_pose, "deactivate_arm_pose");
    }
  ...
  void RobotArmController::write_pose_to_hardware(
      const std::vector<double> & pose, const char * pose_name)
  {
    size_t num_joints = joint_names_.size();
    size_t num_interfaces = params_.interface_names.size();

    if (pose.size() != num_joints)
    {
      RCLCPP_WARN(
          get_node()->get_logger(),
          "%s size (%zu) != joints count (%zu), skipping",
          pose_name, pose.size(), num_joints);
      return;
    }
    ...
    for (size_t j = 0; j < num_joints; ++j)
    {
      ...
      for (size_t i = 0; i < num_interfaces; ++i)
      {
        ...
        if (iface_name == "position")
        {
          command_interfaces_[idx].set_value(pose[j]);
        }
        else if (iface_name == "duration")
        {
          command_interfaces_[idx].set_value(2000.0);  // default 1000ms
        }
      }
    }
  }
      ...
      std::shared_ptr<robot_arm_controller::srv::SetJointAngles::Response> response)
  {
    ...
    for (size_t j = 0; j < num_joints; ++j)
    {
      ...
      for (size_t i = 0; i < num_interfaces; ++i)
      {
        ...
        {
          command_interfaces_[idx].set_value(1500.0);  // default 1000ms
        }
      }
    }
    // Track positions
